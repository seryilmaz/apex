                                   ...
                                   ) 
{
  ...
  const float beta_bmm       = 0.0;
  const float beta_gemm      = 1.0;
  ...
  // Input Linear Fwd
  input_lin_results.copy_(input_bias);
  ...
  THCublasCheck(cublasGemmEx(handle,
                             ...
                             CUDA_R_16F, 
                             embed_dim, 
                             static_cast<const void*>(&beta_gemm),
                             q_lin_results_ptr,
                             CUDA_R_16F, 
                             output_lin_dim,
                             CUDA_R_32F,
                                   ...
                                   ) 
{
  ...
  torch::Tensor input_bgrad         = torch::empty_like(input_bias);
  torch::Tensor output_bgrad        = torch::empty_like(output_bias);
  ...
  THCublasCheck(cublasGemmEx(handle,
                             ...
                             CUBLAS_GEMM_DEFAULT_TENSOR_OP));
  // bgrad
  output_bgrad = output_grads.view({-1, embed_dim}).sum(0, false); 
  ...
  input_bgrad = input_lin_output_grads.view({-1, output_lin_dim}).sum(0, false);
  THCublasCheck(cublasSetMathMode(handle, CUBLAS_DEFAULT_MATH));
