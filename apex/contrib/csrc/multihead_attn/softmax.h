...
__global__ void additive_masked_softmax_dropout_warp_forward(output_t *dst, output_t *softmax_results, uint8_t *dropout_mask, const input_t *src, const input_t *pad_mask, int batch_size, int stride, int element_count, int pad_batch_stride, std::pair<uint64_t,uint64_t> seeds, float p, bool is_training)
{
    ...
    for (int i = 0;i < WARP_BATCH;++i) {
        ...
        for (int it = 0;it < WARP_ITERATIONS;it += ELEMENTS_PER_LDG_STG) {
            int element_index = ELEMENTS_PER_LDG_STG * local_idx + it * WARP_SIZE;
            //int vector elements = ((WARP_ITERATIONS - it) < ELEMENTS_PER_LDG_STG) ? (WARP_ITERATIONS - it) : ELEMENTS_PER_LDG_STG;
            #pragma unroll
            for (int element = 0;element < ELEMENTS_PER_LDG_STG;++element) {
...
__global__ void masked_scale_softmax_warp_backward(output_t *gradInput, const input_t *grad, const input_t *softmax_input, const input_t *pad_mask, const input_t *output, const uint8_t *mask, acc_t scale, int batch_size, int stride, int pad_batch_stride, int element_count)
{
    ...
    // the first element to process by the current thread
    int thread_offset = first_batch * stride + local_idx;
    int thread_offset_vector =  first_batch * stride + ELEMENTS_PER_LDG_STG * local_idx;
